...
#filename = 'D:/桌面-股票/darnn实验/data-nasdaq100/small/nasdaq100_padding.csv'
...
class da_rnn:
    ...
    def train(self, n_epochs=10):
        ...
        for i in range(n_epochs):
            ...
            if i % 50 == 0:
                y_train_pred = self.predict(on_train=True)  # 28383
                y_test_pred = self.predict(on_train=False)  # 12168
                y_pred = np.concatenate((y_train_pred, y_test_pred))    # 40551
                true = concatenate(((self.y).reshape(self.y.shape[0], 1), self.X), axis=1)
                true = self.scaler.inverse_transform(true)
                # X (40560,)
                # y (40560,)
                self.y = true[:, 0]
                y_train_pred = concatenate((y_train_pred.reshape(y_train_pred.shape[0], 1), self.X[self.T-1: len(y_train_pred) + self.T -1]), axis=1)
                y_train_pred = self.scaler.inverse_transform(y_train_pred)
                y_train_pred = y_train_pred[:, 0]
                y_test_pred = concatenate((y_test_pred.reshape(y_test_pred.shape[0], 1), self.X[self.T + len(y_train_pred)-1: ]), axis=1)
                ...
                plt.savefig('results2/result_%d.png' % i)
                plt.show()
            print(i)
        ...
        plt.plot(range(self.T + len(y_train_pred), len(self.y) + 1), y_test_pred, label='Predicted - Test')
        plt.legend(loc='upper left')
        plt.savefig('results2/final_result.png')
