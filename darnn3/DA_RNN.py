...
class da_rnn:
    ...
    def train(self, n_epochs = 500):
        ...
        self.iter_losses = np.zeros(n_epochs * iter_per_epoch ,dtype=np.int16)
        ...
        for i in range(n_epochs):
            ...
            while j < self.train_size:
                ...
                #    self.logger.info("Epoch %d, j:%d batch_size:%d j/batch_size:%d loss = %3.3f.", i, j, self.batch_size, j/self.batch_size, loss)
                j += self.batch_size
                n_iter += 1
...
io_dir = '/home/wang/Wang/da_rnn3/nasdaq100/nasdaq100'

model = da_rnn(file_data = '{}/small/nasdaq100_padding.csv'.format(io_dir), logger = logger, parallel = True,
              learning_rate = .001)
